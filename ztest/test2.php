			...
			if ( $log[ 'table_suffix' ] != $this->dateYmdH ) {
				...
				if ( empty( $exist ) ) {
					...
					$result = $this->db_log->table ( 'tbl_pull_log' )
					                       ->insert ( $insert );
					$output->writeln ( "---{$insert['table_suffix']}:此小时段无数据 ,插入tbl_pull_log结果{$result}---" );

					//发送异常短信通知
					echo $this->checkServerLogStatus ( $insert[ 'table_suffix' ] . '表不存在', $log[ 'create_date' ], $output );
				...
				if ( empty( $exist ) ) {
					$output->writeln ( "---{$Htable}当前表不存在:此小时段无数据 ,没有操作---" );
					//发送异常短信通知

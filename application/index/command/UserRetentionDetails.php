...
class UserRetentionDetails extends Command
{
    private $db_center;
    private $db_log;
    private $db_report;
    ...
    protected function configure()
    {
        ...
        $this->setName('UserRetentionDetails')// 运行命令时使用 "--help | -h" 选项时的完整命令描述
    ...
    private function _generateLog(Output $output)
    {
        ...
        foreach ($this->channel_id_list as $value) {
            ...
            if (empty($keepReport)) {
                ...
                //获取注册人数
                $add['register_num'] = $this->db_log->table('tbl_register_log')
                ...
                $result = $this->db_report->table('tbl_keep_report')
                    ->insert($add);
                if (empty($result)) {
                    $output->writeln("--{$this->date}渠道 {$value['channel_id']}生成的留存根数据失败--");
                }
                $output->writeln("--{$this->date}渠道 {$value['channel_id']}生成的留存根数据成功--");
            } else {
                $update['register_num'] = $this->db_log->table('tbl_register_log')
                    ->whereBetween('create_time', $this->countWhereBetween)
                    ->where(['channel_id' => $value['channel_id']])
                    ->count();
                //更新注册人数
                $result = $this->db_report->table('tbl_keep_report')
                    ...
                    ->update($update);
                if (empty($result)) {
                    $output->writeln("--{$this->date}渠道 {$value['channel_id']}注册人数没有变化--");
                } else {
                    $output->writeln("--{$this->date}渠道 {$value['channel_id']}注册人数更新成功--");
                }
            }
        }
